...
model = PPO(policy='CnnPolicy', env=AnimalTower(),
            verbose=1, tensorboard_log="./ppo_tf/", batch_size=4)
eval_callback = EvalCallback(eval_env=AnimalTower(), best_model_save_path="./ppo_logs/",
                             log_path="./ppo_logs/", eval_freq=10,
                             deterministic=True, render=False, n_eval_episodes=3)
checkpoint_callback = CheckpointCallback(save_freq=10, save_path='./ppo_logs/',
                                         name_prefix='rotete_move_12')
model.learn(total_timesteps=100, callback=[
